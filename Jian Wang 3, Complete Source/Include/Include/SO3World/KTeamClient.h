...
public:
    ...
    BOOL ChangeMember(
        DWORD dwMemberID, int nSrcGroupIndex, DWORD dwDstMemberID, int nDstGroupIndex
    );

    BOOL IsPlayerInTeam(DWORD dwPlayerID);
...
public:
    ...
    int LuaGetAuthorityInfo(Lua_State* L);
    int LuaSetAuthorityInfo(Lua_State* L);

public:
    DECLARE_LUA_CLASS(KTeamClient);

    DECLARE_LUA_DWORD(TeamID);
    DECLARE_LUA_BOOL(System);
