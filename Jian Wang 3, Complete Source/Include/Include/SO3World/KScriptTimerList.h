...
private:
    ...
    typedef std::map<DWORD, KScriptTimer, less<DWORD>, KTIMER_MAP_ALLOCATOR> KSCRIPT_TIMER_MAP;
    KSCRIPT_TIMER_MAP m_ScriptTimerMap;

	DWORD   m_dwScriptTimerID;
	T*      m_pOwner;
};
...
DWORD KScriptTimerList<T, C>::SetTimer(int nTime, const char* szScriptName, int nParam1, int nParam2)
{
    ...
    std::pair<KSCRIPT_TIMER_MAP::iterator, bool> InsRet;

	assert(m_pOwner);
    assert(szScriptName);
    KGLOG_PROCESS_ERROR(m_ScriptTimerMap.size() < C);
    
    dwTimerID = ++m_dwScriptTimerID;

    InsRet = m_ScriptTimerMap.insert(make_pair(dwTimerID, KScriptTimer()));
