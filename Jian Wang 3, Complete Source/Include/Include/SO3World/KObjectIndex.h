...
public:
    ...
    { 
        m_dwNextID = (DWORD)(dwPrefix << (sizeof(m_dwNextID) * CHAR_BIT - ID_PREFIX_BIT_NUM)) + 1;
    }
...
private:
    typedef std::map<DWORD, T*, std::less<DWORD>, KMemory::KAllocator<std::pair<DWORD, T*> > > KOBJECT_INDEX;
