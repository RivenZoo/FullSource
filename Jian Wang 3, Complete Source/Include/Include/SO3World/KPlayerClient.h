...
#include "KIDCache.h"
#include "../../Source/Common/SO3World/Src/KStartTimeEvaluation.h"
...
private:
    ...
    BOOL RealSend(IKG_Buffer* piBuffer);
...
private:
    ...
    int                 m_nFrameSendCount;
    int                 m_nLastSendCountResetFrame;
    ...
    // 发送数据包缓冲队列
    std::list<IKG_Buffer*>          m_SendBufferList;
...
public:
    ...
    BOOL DoTeamChangeMemberGroupRequest(DWORD dwMemberID, int nDstGroupIndex, DWORD dwDstMemberID);
    ...
    BOOL DoSendMailRequest(
        DWORD dwNpcID, int nReQuestID, char szDestName[], char szTitle[], 
        int nMoney, BYTE byTextData[], size_t uTextLen, 
        DWORD dwBox[KMAIL_MAX_ITEM_COUNT], DWORD dwX[KMAIL_MAX_ITEM_COUNT], 
        int nItemPrice[KMAIL_MAX_ITEM_COUNT]
    );
    BOOL DoGetMailListRequest(DWORD dwStartID);
    BOOL DoQueryMailContent(DWORD dwNpcID, DWORD dwMailID, int nMailType);
    BOOL DoAcquireMailMoneyRequest(DWORD dwMailID);
    BOOL DoAcquireMailItemRequest(DWORD dwMailID, int nItemIndex);
    ...
    BOOL DoApplyTongRepertoryPageRequest(int nPageIndex, int nLastUpdateCounter);
    ...
    BOOL DoSaveMoneyInTongRequest(int nSaveMoney);
    BOOL DoPayTongSalaryRequest(int nTotalSalary);
    BOOL DoGetTongSalaryRequest();
    // -----------------------帮会仓库------------------------------

    BOOL DoApplyOpenTongRepertoryRequest(DWORD dwNpcID);
    BOOL DoTakeTongRepertoryItemToPosRequest(int nPageIndex, int nPagePos, int nBoxIndex, int nBoxPos, int nLastUpdateCounter);
    BOOL DoTakeTongRepertoryItemRequest(int nPageIndex, int nPagePos, int nLastUpdateCounter);
    BOOL DoPutTongRepertoryItemToPosRequest(int nBoxIndex, int nBoxPos, int nPageIndex, int nPagePos);
    BOOL DoPutTongRepertoryItemRequest(int nBoxIndex, int nBoxPos, int nPageIndex);
    ...
    BOOL DoStackTongRepertoryItemRequest(
        int nPageIndex, int nPagePos, int nBoxIndex, int nBoxPos, int nStackNum, int nLastUpdateCounter
    );
    // -----------------------------------------------------------
    ...
    BOOL DoSetCurrentDesignationRequest(int nPrefix, int nPostfix, BOOL bDisplayFlag);

    // --------------------- 观察点相关 -------------------------->
    BOOL DoCreateViewPointRequest(int nX, int nY, int nLifeFrame);
    BOOL DoRemoveViewPointRequest();
    BOOL DoMoveViewPointRequest(int nX, int nY);

    // --------------------- 反外挂协议 -------------------------->
    BOOL DoApexProtocol(BOOL bIsUserData, BYTE* pbyData, size_t nDataSize);

    // --------------------- 月卡寄售 -------------------------->
    BOOL DoGameCardSellRequest(DWORD dwNpcID,int nTypeIndex, int nPrice, int nDurationTime);
    BOOL DoGameCardBuyRequest(DWORD dwNpcID,DWORD dwID, int nBuyPrice);
    BOOL DoGameCardLookupRequest(DWORD dwNpcID, BYTE byCardType, int nStartIndex, BYTE byOrderType, BOOL bDesc);
    BOOL DoGameCardCancelRequest(DWORD dwNpcID,DWORD dwID);

    BOOL DoApplyCharacterThreatRankList(DWORD dwCharacterID);
...
private:
    ...
    // 设置生活技能调整等级
    void OnSyncProfessionAdjustLevel(BYTE* pbyData, size_t nSize, int nFrame);
	// 同步更新玩家生活技能
    ...
    void OnSyncMailContent(BYTE* pbyData, size_t nSize, int nFrame);
    void OnReleaseMailMoney(BYTE* pbyData, size_t nSize, int nFrame);
    void OnReleaseMailItem(BYTE* pbyData, size_t nSize, int nFrame);
    ...
    void OnSyncTongRepertoryPageRespond(BYTE* pbyData, size_t nSize, int nFrame);
    ...
    void OnApplyOpenTongRepertoryRespond(BYTE* pbyData, size_t nSize, int nFrame);
    void OnSyncTongHistoryRespond(BYTE* pbyData, size_t nSize, int nFrame);
    //--------------------------------------------------------------------------
    ...
    void OnSyncAchievementCount(BYTE* pbyData, size_t nSize, int nFrame);

    void OnSyncDesignationData(BYTE* pbyData, size_t nSize, int nFrame);
    void OnSetGenerationNotify(BYTE* pbyData, size_t nSize, int nFrame);
    void OnAcquireDesignation(BYTE* pbyData, size_t nSize, int nFrame);
    void OnDesignationAnnounce(BYTE* pbyData, size_t nSize, int nFrame);
    void OnRemoveDesignation(BYTE* pbyData, size_t nSize, int nFrame);
    void OnSyncPlayerDesignation(BYTE* pbyData, size_t nSize, int nFrame);

    void OnSyncMaxLevel(BYTE* pbyData, size_t nSize, int nFrame);
    void OnUseItemPrepare(BYTE* pbyData, size_t nSize, int nFrame);

    //------------------------观察点相关------------------------------
    void OnCreateViewPointRespond(BYTE* pbyData, size_t nSize, int nFrame);
    void OnRemoveViewPointRespond(BYTE* pbyData, size_t nSize, int nFrame);
    void OnMoveViewPointRespond(BYTE* pbyData, size_t nSize, int nFrame);

    void OnAccountEndOfDayTime(BYTE* pbyData, size_t nSize, int nFrame);
    void OnSyncFreeLimitFlagInfo(BYTE* pbyData, size_t nSize, int nFrame);

    void OnApexProtocol(BYTE* pbyData, size_t nSize, int nFrame);

    void OnGameCardSellRespond(BYTE* pbyData, size_t nSize, int nFrame);
    void OnGameCardBuyRespond(BYTE* pbyData, size_t nSize, int nFrame);
    void OnGameCardLookupRespond(BYTE* pbyData, size_t nSize, int nFrame);
    void OnGameCardCancelRespond(BYTE* pbyData, size_t nSize, int nFrame);

    void OnSyncCoin(BYTE* pbyData, size_t nSize, int nFrame);
    
    void OnSeekMentorYell(BYTE* pbyData, size_t nSize, int nFrame);
    void OnSeekApprenticeYell(BYTE* pbyData, size_t nSize, int nFrame);
    void OnSyncMentorData(BYTE* pbyData, size_t nSize, int nFrame);
    void OnDeleteMentorData(BYTE* pbyData, size_t nSize, int nFrame);
    void OnSyncPlayerCampFlag(BYTE* pbyData, size_t nSize, int nFrame);

    void OnCharacterThreatRanklistRespond(BYTE* pbyData, size_t nSize, int nFrame);
