...
#ifdef _SERVER
    template <class TFunc> BOOL TraverseViewPoint(TFunc& Func, KRegion* pCenter = NULL);
#endif
	// 遍历Region中的Cell
...
#ifdef _SERVER
template <class TFunc> 
BOOL KRegion::TraverseViewPoint(TFunc& Func, KRegion* pCenter /* = NULL*/)
{
    BOOL                    bResult    = false;
    BOOL                    bRetCode   = false;
    KVIEW_POINT*            pViewPoint = (KVIEW_POINT*)m_ViewPointList.GetHead();

    while (pViewPoint != NULL)
    {
        KPlayer* pPlayer = (KPlayer*)(pViewPoint->pPlayer);
        KGLOG_PROCESS_ERROR(pPlayer);

        if (pCenter && pPlayer->IsInViewRangeByPlayer(pCenter))
            continue;

        bRetCode = Func(pPlayer);
        KG_PROCESS_ERROR(bRetCode);

        pViewPoint = (KVIEW_POINT*)pViewPoint->GetNext();
    }
...
{
    KRegisterNewNpcFunc(KNpc* pNpc, KRegion* pRegion) : m_pNpc(pNpc), m_pFromRegion(pRegion) {};
    BOOL operator () (KPlayer* pPlayer);
    KNpc*       m_pNpc;
    KRegion*    m_pFromRegion;
};
