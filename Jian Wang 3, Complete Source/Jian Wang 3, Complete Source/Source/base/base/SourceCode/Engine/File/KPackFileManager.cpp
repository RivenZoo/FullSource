...
/**
 * @brief 查找子文件在包中的位置,如果找到返回true,uIndex为传回找到的位置；未找到则返回false,uIndex传回应当插入的位置
 * @param uElemId 文件的ID，hash之后得到的
 * @param nPakIndex 所使用的pak引用
 * @param uIndex 为传回找到的位置，引用
 * @return 未找到则返回false,uIndex传回应当插入的位置
 */
bool KPackFileManager::FindElem(unsigned int uElemId, int nPakIndex, unsigned int& uIndex)
{
    ...
    {
        nMid = (nBegin + nEnd) / 2;
        if (uElemId < item.pIndexList[nMid].uId)	/* 前提条件是：ID值是按从小到大顺序存储的 */
        {
            nEnd = nMid - 1;
        }
...
/**
 * @brief 分配缓冲区，并把字文件解开读取到该缓冲区
 * @param nPakIndex 当前使用的pack索引
 * @param uElemIndex 找到的需要解压出来的文件索引
 * @param uElemSize 返回该文件的大小（原始大小）
 * @return
 */
unsigned char* KPackFileManager::AllocBufferAndReadElemFile(int nPakIndex, unsigned int uElemIndex, unsigned int& uElemSize)
{
	...
	item.pIOFile->Seek(item.nDataEndOffset, SEEK_SET);	///恢复文件指针位置?
...
/**
 * @breif 从包中读取一段数据
 * @param nPakIndex 正在使用的Pak索引
 * @param uOffset 偏移量：将要读取数据的开始位置，相对item.pIOFile开始位置
 * @param uStorSize 存储的大小，如果压缩了，则是压缩后的大小
 * @param uPakMethod pack方式，none，UCL，Filter
 * @param pBuffer 读取数据的存储空间
 * @param uSize
 * @return 
 */
...
bool KPackFileManager::LoadPackPartner(int nPakIndex)
{
	...
	{
		PACK_ITEM& item = m_PackItemList[nPakIndex];	// 引用
