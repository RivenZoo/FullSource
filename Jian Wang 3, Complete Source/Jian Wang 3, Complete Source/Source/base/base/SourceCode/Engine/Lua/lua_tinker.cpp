...
{
	return lua_tinker::dobuffer(L, buff, strlen(buff), "lua_tinker::dostring()");
}

/*---------------------------------------------------------------------------*/ 
bool lua_tinker::dobuffer(lua_State *L, const char* buff, size_t len, const char *msg)
{
	bool ret = false;
	lua_pushcclosure(L, on_error, 0);
	int errfunc = lua_gettop(L);

    if(luaL_loadbuffer(L, buff, len, msg) == 0)
	{
		if(lua_pcall(L, 0, 0, errfunc) != 0)
		{
			lua_pop(L, 1);
...
static void call_stack(lua_State* L, int n)
{
    ...
    if(lua_getstack(L, n, &ar) == 1)
	{
		...
		lua_getglobal(L, CURRENT_SCRIPT_FILE);
		const char *source = (const char*)lua_tostring(L, -1);

		if(ar.name)
			lua_tinker::print_error(L, "%s%s() : line %d [%s : line %d]", indent, ar.name, ar.currentline, source, ar.linedefined);
		else
			lua_tinker::print_error(L, "%sunknown : line %d [%s : line %d]", indent, ar.currentline, source, ar.linedefined);
...
#endif
		printf("%s\n", text);
		KGLogPrintf(KGLOG_WARNING, "[Lua] %s\n", text);
		lua_pop(L, 1);
	}
}
