...
The original code is: SynHighlighterSample.pas, released 2004-11-09.
Description: Syntax Parser/Highlighter
The initial author of this file is zhujianqiu.
...
$Id: SynHighlighterSample.pas,v 1.1 2005/06/20 03:12:58 zhujianqiu Exp $
...
const
{$ENDIF}
  SYNS_FilterLuaScript = 'Lua files (*lua)|*.lua';
  SYNS_LangLuaScript = 'Lua Script';
  SYNS_AttrTest = 'Test';
...
begin
  ...
  fKeyAttri := TSynHighLighterAttributes.Create(SYNS_AttrOperator);
  ...
  InitIdent;
  MakeMethodTables;
  fDefaultFilter := SYNS_FilterLuaScript;
  fRange := rsUnknown;
end;
...
begin
  Result := fDefaultFilter <> SYNS_FilterLuaScript;
end;

{$IFNDEF SYN_CPPB_1} class {$ENDIF}
function TSynSampleSyn.GetLanguageName: string;
begin
  Result := SYNS_LangLuaScript;
end;
