...
$Id: SynCompletionProposal.pas,v 1.1 2005/06/20 03:13:14 zhujianqiu Exp $
...
begin
  ...
  try
    List.CommaText := S;
    List.Delimiter:=',';  //Added by AQ, 2004.11.12
    for i := 0 to (List.Count div 2 - 1) do
    begin
      if i = CurrentIndex - 1 then
        Result := Result + '\style{~B}' + List[i*2] + ' ' + List[i*2 + 1] + '\style{~B}'
      else
        Result := Result + List[i*2] + ' ' + List[i*2 + 1];
...
begin
  ...
  else if (DisplayType = ctParams) then
    begin
      case Key of
        #32..'z':
          begin
...
begin
  ...
  begin
  //Added by AQ,  2004.11.12
    if (FAssignedList.Count = 0) or (CurrentString = '') then
      begin
        Close;
        Exit;
      end;
  //End Added by AQ
    ...
    begin
      ...
      begin
        if (i + FScrollbar.Position = Position) and (FDisplayKind = ctCode) then
        begin
  ...
  begin
    ...
    begin
      ...
      //GBN 10/11/2001
      i:=Self.FCurrentEditor.CaretX;
      while (i > 0) and (FCurrentString[i] <> '(') do
        dec(i);
      j:=i;
      dec(i);
      while (i > 0) and (FCurrentString[i] in ['_', 'a'..'z', 'A'..'Z', '0'..'9']) do
        dec(i);
      FuncName:=Copy(FCurrentString, i + 1, j - i - 1);

      i:=0;
      for j := 0 to FAssignedList.Count - 1 do
        if (FuncName = InsertList[j]) then
          begin
            AlreadyDrawn := False;
            if Assigned(OnPaintItem) then
            ...
            begin
              if FDisplayKind = ctParams then
                begin
                  ParamIndex:=1;
                  for k:=1 to Length(FCurrentString) do
                    if FCurrentString[k] = ',' then
                      ParamIndex:=ParamIndex + 1;

                  TmpString := FormatParamList(FAssignedList[j], ParamIndex)

                end
              else
                TmpString := FAssignedList[j];
...
procedure TSynBaseCompletionProposalForm.RecalcList;
...
{    if UseInsertList then
    ...
    CompareString := Copy(CompareString, 1, Length(CurrentString));

    if FAnsi then
    begin
      if FCase then
        Result := AnsiCompareStr(CompareString, CurrentString) = 0
      else
        Result := AnsiCompareText(CompareString, CurrentString) = 0;
    end else
    begin
      if FCase then
        Result := CompareStr(CompareString, CurrentString) = 0
      else
        Result := CompareText(CompareString, CurrentString) = 0;
    end;
  end;
...
procedure TSynBaseCompletionProposal.ExecuteEx(s: string; x, y: integer; Kind : SynCompletionType);
  ...
  begin
    ...
    ctParams:
      begin
        BorderWidth := 2;
//        tmpHeight := Form.FHeightBuffer + Form.FEffectiveItemHeight * FForm.FAssignedList.Count +
//            BorderWidth;
        tmpHeight := Form.FEffectiveItemHeight * 3 + BorderWidth
          + 2 * Form.Margin;
        tmpy:=tmpy - tmpHeight - 20;
...
begin
  ...
  ctCode:
    begin
      ...
      if (Self.FForm.FAssignedList.Count > 0) then
        Form.Show;
    end;
  ctHint, ctParams:
    begin
...
procedure TSynCompletionProposal.Deactivate;
begin
  inherited;
  FNoNextKey:=false;
end;
...
begin
  ...
  begin
    s := AEditor.LineText;
    i := AEditor.CaretX - 1;
    if (i <= Length(s)) or ((DefaultType = ctParams) and (s <> '')) then
    begin
      FAdjustCompletionStart := False;
      while (i > 0) and (i <= Length(s)) and
            ((s[i] in ['_', 'a'..'z', 'A'..'Z', '0'..'9'])) and
            (not (s[i] in Form.FWordBreakChars)) do
        dec(i);

      FCompletionStart := i+1;
      if DefaultType = ctParams then
        Result := s
       else
