...
texture tShadowmap1;
samplerCUBE SamplerShadow1 = sampler_state
{
    Texture   = <tShadowmap1>;
    MipFilter = NONE;
    MinFilter = Point;
...
{
   float depth = texCUBE(SamplerShadow1,V).r;
   float S = 1;
   if( L -  depth > L * 0.01 )
        S = 0;
    return S;
          ...
          ) : COLOR
{
   ...
   float L = length(V);
   float depth = texCUBE(SamplerShadow1,V).r;
   float def = max(0,L - depth);
 
   float S=0;
   for(int i=0;i<64;i++)
   {
      float cc = max(5,def *def*0.00001);
      S += shadow(V+vUSNoise[i]*cc,L) ;
   }
    S /= 64;
    
   float dis = max(0,1-L / vPointLightPosition.w);
   float t = max(0,dot(-nor,normalize(V)));

   r.rgb = t * S * dis * vPointLightColor.rgb * vPointLightColor.a;
   r.a = 1;
    return r;
}
