...
private:
	// 计算扩展缓冲区字节数
	inline size_t CalcExtendBuffer(size_t nFreeBytes, size_t nBuffBytes)
	{
		return (nFreeBytes > nBuffBytes ? nFreeBytes + (nBuffBytes << 1) : (nBuffBytes << 1));
	}
	//*********************************************************************
	// description	: 扩展缓冲区，使其满足使用要求 
	// parameter	: nFreeBytes[in] 需要的空闲缓冲区字节数
	// return		: BOOL 成功返回TRUE，失败返回FALSE
	//*********************************************************************
	BOOL ExtendBuffer(size_t nFreeBytes);
