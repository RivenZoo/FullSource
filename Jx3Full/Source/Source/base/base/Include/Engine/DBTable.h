...
public:
	bool bStop;
	ZDBTable(const char *path, const char *name, int nFlag = ZDBTABLE_CACHE, BOOL bIsRelativePath = TRUE);   //环境目录和数据表的名字
	virtual ~ZDBTable();
...
//下面是一些维护性的操作
	...
	void log_archlist(const char *pszFolder);
	//*********************************************************************
	// function		: log_archlist
	// parameter	: pDbEnv  数据库环境
	// parameter	: pszPath 目标路径
	// return		: void
	// comment		: 把数据库文件和日志文件复制到指定目录下，并将原来不再使用的日志文件删除。
	//*********************************************************************
	static void log_archlist(DB_ENV *pDbEnv, LPCSTR pszPath);
	//*********************************************************************
	// function		: SnapShot
	// parameter	: pDbEnv  数据库环境
	// parameter	: pszPath 目标路径
	// parameter	: pszDatabase 数据库名字
	// return		: void
	// comment		: 把数据库文件和日志文件复制到指定目录下，并将原来不再使用的日志文件删除。
	//*********************************************************************
	static void SnapShot(DB_ENV *pEnv, LPCSTR pszPath, LPCSTR pszDatabase);
};
