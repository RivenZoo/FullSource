...
#endif
    KTimeLimitationBindItemMgr m_TimeLimitationBindItemMgr;
...
public:
    ...
    void SetViewObjectRegisterFlag(KRegion* pRegion, BOOL bFlag);
    BOOL RegisterPlayerToArea(KPlayer* pPlayer, KRegion* pCenter);
    BOOL RegisterAreaObjectToPlayer(KPlayer* pPlayer, KRegion* pCenter);
#endif //_SERVER
...
public:
    ...
    int LuaTimeLimitationBindItemAddItem(Lua_State* L);
    int LuaTimeLimitationBindItemDelItem(Lua_State* L);
    int LuaTimeLimitationBindItemAddPlayer(Lua_State* L);
    int LuaTimeLimitationBindItemRemovePlayer(Lua_State* L);
    int LuaTimeLimitationBindItemGetLeftTime(Lua_State* L);
    int LuaTimeLimitationBindItemGetItemOwnerID(Lua_State* L);
    int LuaTimeLimitationBindItemGetAllPlayer(Lua_State* L);
...
// 遍历Scene中的Doodad
template <class TFunc> 
BOOL KScene::TraverseDoodad(TFunc& Func)
{
    BOOL bResult  = false;
    BOOL bRetCode = false;
...
#define BROADCAST_TO_REGION(nX, nY)                             \
    do {                                                        \
        KRegion* pRegion = GetRegion((nX), (nY));               \
        if (pRegion)                                            \
        {                                                       \
            pRegion->TraversePlayer(BroadcastFunc);             \
            pRegion->TraverseViewPoint(BroadcastFunc, pCenter); \
        }                                                       \
    } while (false)
...
void KScene::Broadcast(KRegion* pCenter, TFunc& BroadcastFunc, int nRange)
{
    ...
    BroadcastFunc.m_nMaxSyncCount = MAX_SYNC_PER_REGION *  (2 * nRange + 1) * (2 * nRange + 1);

    // Broadcast to center region
    pCenter->TraversePlayer(BroadcastFunc);
    pCenter->TraverseViewPoint(BroadcastFunc, pCenter);

    for (int i = 1; i <= nRange; i++)
    {
        // Broadcast to 4 edges.
        for (int j = -i; j < i; j++)
        {
            BROADCAST_TO_REGION(pCenter->m_nRegionX + j, pCenter->m_nRegionY + i);  // Top
            BROADCAST_TO_REGION(pCenter->m_nRegionX - i, pCenter->m_nRegionY + j);  // Left
            BROADCAST_TO_REGION(pCenter->m_nRegionX - j, pCenter->m_nRegionY - i);  // Bottom
            BROADCAST_TO_REGION(pCenter->m_nRegionX + i, pCenter->m_nRegionY - j);  // Right
        }
    }
}
   ...
   for (int nX = nLeft; nX <= nRight; nX++)
    {
        for (int nY = nBottom; nY <= nTop; nY++)
        {
            ...
            if (pRegion)
            {
                ...
                pRegion->TraverseViewPoint(Func, pRegion);
            }
        }
    }
}
