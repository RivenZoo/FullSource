...
#endif
#ifdef BLOCK_SMOOTHING_SUPPORTED
LOCAL(boolean_def) smoothing_ok JPP((j_decompress_ptr cinfo));
...
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  boolean_def smoothing_useful = FALSE;
  int ci, coefi;
  jpeg_component_info *compptr;
  JQUANT_TBL * qtable;
  int * coef_bits;
...
decompress_smooth_data (j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  ...
  jpeg_component_info *compptr;
  inverse_DCT_method_ptr inverse_DCT;
  boolean_def first_row, last_row;
  JBLOCK workspace;
  int *coef_bits;
  JQUANT_TBL *quanttbl;
...
GLOBAL(void)
jinit_d_coef_controller (j_decompress_ptr cinfo, boolean_def need_full_buffer)
{
  my_coef_ptr coef;
